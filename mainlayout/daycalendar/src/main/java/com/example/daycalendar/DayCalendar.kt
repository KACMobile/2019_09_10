    ...
    : ConstraintLayout(context,attrs,defStyleAttr)
{
    ...
    val hollydays = arrayOf(hollyday(1,1, "?ã†?†ï", false), hollyday(1, 1, "?Ñ§?Ç†", true),
        hollyday(3, 1, "?Çº?ùº?†à", false), hollyday(5, 5, "?ñ¥Î¶∞Ïù¥?Ç†", false),
        hollyday(4, 8, "Î∂?Ï≤òÎãò ?ò§?ã†?Ç†", true),
        hollyday(6, 6, "?òÑÏ∂©Ïùº", false), hollyday(8, 15, "Í¥ëÎ≥µ?†à", false),
        hollyday(8, 15, "Ï∂îÏÑù", true), hollyday(10, 3, "Í∞úÏ≤ú?†à", false),
        hollyday(10, 9, "?ïúÍ∏??Ç†", false), hollyday(12, 25, "?Å¨Î¶¨Ïä§ÎßàÏä§", false))

    var changedCell= arrayListOf<TextView>()
    lateinit var saveDataSnap: DataSnapshot //DataSnapshot?ùÑ Î∞õÏúºÎ©? set?ï®
    var followListSnapshot = arrayListOf<DataSnapshot>() //followList DataSnapshot?ùÑ Î∞õÏúºÎ©? add?ï®
    ...
    fun calendardefaultsetting() {
        ...
        } // ?öî?ùº Î∞∞Í≤Ω?Éâ Ï¥àÍ∏∞?ôî

        for (i in 0..23) {
            timeRow[i].setBackgroundColor(Color.WHITE)
        } // ?ãúÍ∞? Î∞∞Í≤Ω?Éâ Ï¥àÍ∏∞?ôî

        dayText[currentDOW-1].setBackgroundColor(Color.YELLOW) // current ?öî?ùº Î∞∞Í≤Ω?Éâ ?Ñ§?†ï

        timeRow[currentTime].setBackgroundColor(Color.YELLOW) // current ?ãúÍ∞? Î∞∞Í≤Ω?Éâ ?Ñ§?†ï
